...
//    }
    ...
    public int updateOrderDetail(int id, OrderDetail orderDetail){
        Query query = sessionFactory.getCurrentSession().createQuery(
                "update OrderDetail set productID=:productId, quantity=:quantity, price=:price where id=:id"
        );
        query.setParameter("productId", orderDetail.getProductID());
        query.setParameter("quantity", orderDetail.getQuantity());
        query.setParameter("price", orderDetail.getPrice());
    ...
    public int deleteOrderDetail(int id){
        OrderDetail existedOrderDetail = (OrderDetail) sessionFactory.getCurrentSession().get(OrderDetail.class, id);
        existedOrderDetail.getOrderID().getOrderDetailList().remove(existedOrderDetail);
        sessionFactory.getCurrentSession().delete(existedOrderDetail);
        return existedOrderDetail.getId();
    }
}
